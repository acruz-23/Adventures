...
function generateActivityCategory(array) {
  ...
  for (let categories of array) {
    const catergoryOptions = new Option(categories, categories);
...
function createActivityTable(myTable, activity) {
  ...
  cell2.textContent = activity.name;
  cell3.innerHTML = activity.description;
...
function displayTable(category, activitiesArray) {
  ...
  if (activitiesArray.length === 0) {
    ...
    activityTableEL.style.display = "table-header-group";
  }
}
